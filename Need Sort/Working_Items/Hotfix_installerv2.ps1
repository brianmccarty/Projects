...
If ($64bit){
    Copy-Item -Path \\$env.user\netlogon\Hotfixes\*x64.msu -Destination C:\cpihotfixes
    Publish-HipChatRoomMessage -apitoken fb6a882a2874cbb512b45a7014f0ef -roomid 3455255 -from $pcname -message "Copying x64 files"
    }
Else {
    Copy-Item -Path \\$env.user\netlogon\Hotfixes\*x86.msu -Destination C:\cpihotfixes
...
$cpihotfixes = Get-Item -Path C:\cpihotfixes\* | select Name
$cpihotfixes

foreach ($cpihotfix in $cpihotfixes) {
    Publish-HipChatRoomMessage -apitoken fb6a882a2874cbb512b45a7014f0ef -roomid 3455255 -from $pcname -message "Starting $cpihotfix"
    $arglist = "C:\Windows\System32\wusa.exe $cpihotfix /quiet /norestart"
    Start-Process $arglist -NoNewWindow -Wait
    Publish-HipChatRoomMessage -apitoken fb6a882a2874cbb512b45a7014f0ef -roomid 3455255 -from $pcname -message "Finished $cpihotfix"
    }
