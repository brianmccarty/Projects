  ...
  while (!$x){                                                                                               
    ...
    switch ($e.targetinstance) 
      ...
      {$_.message -like "*member was added*"} {
        $member= $e.targetinstance.insertionstrings[0] | % { [regex]::Match($_,'([^=]+),OU=').Groups[1].Value } | foreach-object {$_ -replace "\\", ""} 
      ...
      {($_.message -like "*directory service object was created*") -and ($_.message -like "*organizationalUnit*")} {
        $subject="OU Created - $($($e.targetinstance.insertionstrings[8]) | % { [regex]::Match($_,'OU=([^,]+)').Groups[1].Value }) - By $($e.targetinstance.insertionstrings[3]) - Event $($_.eventcode)";$body="$($_.computername) `n$($_.message)"
        Write-Output $subject $body
        #send-mailmessage -from $email -to $email -subject $subject -body $body -smtpserver $smtp
        } 
      {($_.message -like "*directory service object was deleted*") -and ($_.message -like "*organizationalUnit*")} {
        $subject="OU Deleted - $($($e.targetinstance.insertionstrings[8]) | % { [regex]::Match($_,'OU=([^,]+)').Groups[1].Value }) - By $($e.targetinstance.insertionstrings[3]) - Event $($_.eventcode)";$body="$($_.computername) `n$($_.message)"
        Write-Output $subject $body
        #send-mailmessage -from $email -to $email -subject $subject -body $body -smtpserver $smtp
        } 
      {$_.message -like "*directory service object was moved*"} {
        $subject="Object Moved - $($e.targetinstance.insertionstrings[8] | % { [regex]::Match($_,'([^=]+),OU=').Groups[1].Value }) - By $($e.targetinstance.insertionstrings[3]) - Event $($_.eventcode)";$body="$($_.computername) `n$($_.message)"
        Write-Output $subject $body
        #send-mailmessage -from $email -to $email -subject $subject -body $body -smtpserver $smtp
        } 
      {($_.message -like "*user account was enabled*") -and ($($e.targetinstance.insertionstrings[0]) -notmatch "\`$")} {
        ...
        if ($enabled -ne "no"){Write-Output $subject $body};$enabled="yes"
        } 
      ...
      {($_.message -like "*reset an account*") -and ($($e.targetinstance.insertionstrings[0]) -notmatch "\`$")} {
        ...
        if ($reset -ne "no"){Write-Output $subject $body};$reset="yes"
        } 
      ...
      {($_.message -like "*directory service object was modified*") -and ($_.message -like "*groupPolicyContainer*") -and ($_.message -like "*versionNumber*") -and ($_.message -like "*Value Added*")} {
        ...
        if ($modified -ne "no"){Write-Output $subject $body};$modified="yes"
        } 
